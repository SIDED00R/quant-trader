...
def decide(qty: Decimal, price: Decimal, cash: Decimal, equity: Decimal,
    ...
    규칙 본체는 decision_record.classify (사유·수치까지 산출하는 단일 출처). 여기선 실행할 BUY/SELL 만 추린다.
    """
    d = classify(qty, price, cash, equity, target_w, band)
    return (d.decision, d.quantity) if d.decision in ("BUY", "SELL") else None
