...
매 봉: 최신가 갱신 → strategy.on_tick → 평가자산으로 봉 단위 MDD(정확)와 final_equity 갱신.
...
class BacktestEngine:
    def __init__(self, account: BacktestAccount, fills: FillModel, equity_sample_sec: float = 60.0):
        ...
        self.max_drawdown = Decimal(0)                       # 봉 단위 정확 MDD(비율)
        self.final_equity = account.initial_cash             # 마지막 봉의 실제 평가자산(수익률용)
        self.n_bars = 0
    ...
    def run(self, ticks, strategy) -> None:
        for t in ticks:
            ...
            # 균등 그리드 표본: 그리드 시점의 평가자산은 직전 봉 값(_last_eq)을 carry-forward(룩어헤드 없음)
