"""주식 분봉 백필 실행 (단일 책임: CLI → toss_intraday → ClickHouse stock_candles_1m).

예) .venv/Scripts/python -m batch.backtest.backfill_stock_intraday --symbols 005930,000660,AAPL --days 30
토스증권 분봉(1m, KR/US)을 받아 ClickHouse stock_candles_1m에 적재한다(인트라데이 백테스트 입력).
사전조건: docker compose up -d clickhouse + python -m scripts.init_db (stock_candles_1m 생성),
...
from batch.backtest.toss_intraday import fetch_minute, upsert_clickhouse
...
def main(argv=None) -> int:
    try:
        ...
        pass
    p = argparse.ArgumentParser(description="토스증권 주식 분봉 백필 → ClickHouse stock_candles_1m")
    p.add_argument("--symbols", default=_DEFAULT_SYMBOLS, help="쉼표 구분 (KR 6자리 / US 티커)")
    p.add_argument("--days", type=int, default=30, help="과거 일수(분봉은 무거움 — 기본 30일)")
    p.add_argument("--table", default="stock_candles_1m", help="대상 ClickHouse 테이블")
    a = p.parse_args(argv)
