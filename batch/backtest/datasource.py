...
table=stock_candles_1d면 주식 일봉(toss_daily 적재본, KR+US) — 동일 스키마(symbol/window_start/close).
...
_TABLES = {"candles_1m", "candles_1d", "stock_candles_1d"}  # 허용 테이블(SQL 식별자 파라미터화 불가 → 화이트리스트로 주입 차단)
...
def load_clickhouse_candles(symbols=None, start=None, end=None, table="candles_1m"):
    ...
    table: 'candles_1m'(기본)·'candles_1d'(장기 일봉)·'stock_candles_1d'(주식 일봉). ReplacingMergeTree 중복 정리를 위해 항상 FINAL.
    """
