...
## 1.5단계 — 거래빈도·수수료 제어 (baseline 진단 반영) — ⏭️ 다음 작업
> baseline(−97.97%)의 주범은 신호가 아니라 과매매·수수료(자본의 83%). 신규 알고리즘 전에 공통 레버부터.
- [ ] 시간 기반 가드(쿨다운/최소보유/워밍업)를 **봉 단위**로 재튜닝 (1분봉에서 무력화됨)
- [ ] 수수료 인지 필터(최소 기대수익 > 왕복 수수료)로 과매매 차단
- [ ] 데드크로스 청산 재검토 (baseline 최대 출혈원 −22.5M; TAKE/TRAIL은 흑자)
- [ ] 재튜닝 후 SMA baseline 재측정 → 공정 기준선 (목표: 거래수·수수료 대폭↓)
