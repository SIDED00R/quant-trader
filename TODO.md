...
### 3.1 백테스트 하니스 보강 (#9)
- [ ] 상위 타임프레임 리샘플링(`--bar-min`, 예 일봉=1440) — 신호·거래 빈도 구조적 축소
- [ ] train/test 분할 + **walk-forward**(롤링 IS/OOS) 러너 — 과적합·룩어헤드 방지
- [ ] **Deflated Sharpe**(시도 수 N 보정) 보고 — 다중 시도 선택편향 교정

### 3.2 저회전 전략 설계·구현 (#10)
- [ ] **추세추종 long-or-cash**: 일봉 ~10/40일 SMA, 상향 보유·하향 현금화(공매도X). 매도기준 전까지 홀딩
- [ ] **변동성 타게팅 사이징**: 최근 변동성 역수 비중(저회전 변형: 표준편차/AR(1) 예측/레버리지 캡)
- [ ] **레짐·변동성 필터**: 고변동·횡보장 노출 축소(추세장만 진입)
- [ ] **비용 인지 trade-generation**: 기대수익 > 왕복비용일 때만(신호→거래 변환부에서 churn 차단)
- [ ] (선택) 청산 임계값 **OU Monte-Carlo** 산출(과거 단일경로 grid-search 과적합 회피)

### 3.3 유니버스 선정
- [ ] 전 종목(~180) X → 주요·유동성 소수(BTC/ETH 등). 종목별 성과로 만성손실 알트 제외

### 3.4 선별 & 다음
- [ ] 재설계 전략 walk-forward 측정 → **OOS 양수 수익** 확인(성공기준), Deflated Sharpe로 검증
- [ ] 통과 전략 + 초기 가중치 확정 → 4단계 앙상블로
