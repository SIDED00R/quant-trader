...
## 다음 — 주식 확장 (진행)
코인에서 검증한 **동일 엔진·전략·검증 하니스를 주식으로 재사용**한다. 백테스트 엔진을 자산군 일반화(코인=소수 수량, 주식=정수 주·국내 매도 거래세 0.20%, `common/market_hours`로 자산군·장시간 판정)하고 주식 일봉(`stock_candles_1d`)을 백테스트 데이터소스로 연결했다(#120/#122). **코인 경로는 무영향**. 분업: 데이터=토스/업비트, 체결=KIS(주식)/시뮬(코인). 주식 전략 선별·유니버스 확정은 다음 단계.
