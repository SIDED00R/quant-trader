...
from common import log, rate_limit
...
def send_message(token: str, chat_id, text: str) -> None:
    ...
    try:
        ...
        logger.error(f"sendMessage 실패: {type(e).__name__}: {e}")
...
def send_photo(token: str, chat_id, png: bytes, caption: str) -> None:
    ...
    try:
        ...
        if r.status_code != 200:
            logger.warning(f"sendPhoto {r.status_code}: {r.text[:200]}")
    except Exception as e:
        logger.error(f"sendPhoto 실패: {type(e).__name__}: {e}")
...
def _handle_update(token: str, upd: dict, index: dict) -> None:
    ...
    if chat_id not in TELEGRAM_ALLOWED_CHAT_IDS:    # 빈 화이트리스트 = 전면 거부(fail-closed — 문서 기본값)
        logger.warning(f"비허용 chat_id={chat_id} drop")
        return
...
def main() -> int:
    ...
    if not token:
        logger.warning("TELEGRAM_BOT_TOKEN 미설정 — 유휴(재시작 루프 방지). 토큰 주입 후 컨테이너 재시작 필요.")
        while True:                       # exit 0 시 restart:unless-stopped 루프 방지 → 유휴 대기
            time.sleep(3600)
    index = load_index()
    offset, backoff = 0, 1
    logger.info(f"시작 — 허용 chat {len(TELEGRAM_ALLOWED_CHAT_IDS)}개, 종목명 {len(index.get('rows', []))}개")
            ...
            try:
                ...
                for upd in r.json().get("result", []):
                    ...
                    try:
                        ...
                        logger.exception("텔레그램 업데이트 처리 실패")
                backoff = 1
            except Exception as e:                           # 409(다중 소비자)·네트워크 등 → 백오프 후 계속(크래시 금지)
                logger.error(f"poll 오류(재시도): {type(e).__name__}: {e}")
