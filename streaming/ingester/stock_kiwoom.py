...
async def run() -> None:
    ...
    if not (KIWOOM_APP_KEY and KIWOOM_APP_SECRET):
        logger.warning("KIWOOM_APP_KEY/SECRET 미설정 — 주식 수집 비활성(idle). .env 설정 후 재시작.")
        while True:
    ...
    logger.info(
        f"connecting Kiwoom | kafka={KAFKA_BOOTSTRAP_SERVERS} ws={KIWOOM_WS_URL}"
    )
    backoff = 1
    count = 0
                ...
                async with websockets.connect(KIWOOM_WS_URL, ping_interval=None) as ws:
                    ...
                    while True:
                        ...
                        if trnm == "LOGIN":
                            ...
                            logger.info(
                                f"LOGIN ok → REG {len(STOCK_SYMBOLS)} symbols"
                            )
                            continue
                        if trnm == "REG":
                            if str(msg.get("return_code")) not in ("0", "None"):
                                logger.warning(f"REG 경고: {msg.get('return_msg')}")
                            continue
                        if trnm != "REAL":
                            continue
                        for item in msg.get("data", []):
                            ...
                            try:
                                ...
                                logger.warning(f"skip bad real: {e}")
                                continue
                            try:
                                producer.produce(
                                    TOPIC_STOCK_TICKS,
