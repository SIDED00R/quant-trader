...
**폴더당 하나의 기능, 같은 깊이 = 같은 성격** — 하위 패키지 기준:
- `streaming/` 수집→집계 · `trading/` 신호→체결(`strategy/{core,plugins,runners}` — 프레임워크/전략 구현/엔트리포인트)
- `batch/` 오프라인(`backtest`=코어 · `candles`=시세 수집·백필 · `rawdata`=외부 원본 수집 · `jobs`=운영 잡 · `features`·`ml` · `universe`=지수 데이터)
- `common/` 공용 라이브러리(평면=기반 인프라, `broker/`·`marketdata/`·`chart/`·`equity/` 하위 묶음) · `api/` 서빙
- 테스트는 최상위 `tests/`에 소스 트리 미러로.
- 기존 파일 확장보다 **새 파일**을 선호. 공통 로직 중복 시 `common/`으로 추출.

## 깨지면 안 되는 경계

- **프로덕션 app 이미지(`Dockerfile`)는 `batch/`를 제외**한다. 따라서 거기에 적재·실행되는 `common/`·`streaming/`·`trading/`·`api/`의 상시 경로는 **`batch.*`를 import 하지 않는다**. 특히 `common/marketdata/candles.py`는 backtest 비의존(프로덕션 일봉 로더). **예외**: `trading/strategy/runners/stock_trade_once.py`·`us_trade_once.py`·`stock_trade_common.py`·`kr_ichimoku_trade_once.py`는 `Dockerfile.batch`(trade 프로파일) 전용 엔트리포인트라 `batch.ml.stock_score`·`batch.candles.refresh_stock_daily`/`toss_daily`에 의존한다(app 이미지에선 실행되지 않음). 단 새 공용 모듈 `common/marketdata/ichimoku.py`·`common/marketdata/stock_ohlc.py`·`trading/portfolio/paper_ledger.py`는 batch 비의존(app 이미지 안전).
