...
- **프로덕션 app 이미지(`Dockerfile`)는 `batch/`를 제외**한다. 따라서 거기에 적재·실행되는 `common/`·`streaming/`·`trading/`·`api/`의 상시 경로는 **`batch.*`를 import 하지 않는다**. 특히 `common/marketdata/candles.py`는 backtest 비의존(프로덕션 일봉 로더). **예외**: `trading/strategy/runners/stock_trade_once.py`·`us_trade_once.py`·`stock_trade_common.py`·`kr_ichimoku_trade_once.py`는 `Dockerfile.batch`(trade 프로파일) 전용 엔트리포인트라 `batch.ml.stock_score`·`batch.backtest.refresh_stock_daily`/`toss_daily`에 의존한다(app 이미지에선 실행되지 않음). 단 새 공용 모듈 `common/marketdata/ichimoku.py`·`common/marketdata/stock_ohlc.py`·`trading/portfolio/paper_ledger.py`는 batch 비의존(app 이미지 안전).
