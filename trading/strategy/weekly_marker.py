...
완료(=그 주 마커 기록) 조건은 '체결기반' — 신규 매수가 없거나(이미 목표 정렬) 실제 체결이 1건 이상일 때만.
...
def completed(plan: dict, placed: list) -> bool:
    """이번 주 리밸런싱이 '완료'됐는지 — 신규 매수가 없거나(이미 정렬) 실제 체결이 1건 이상."""
    if not plan["targets"]:        # 랭킹 산출 실패(데이터 결손 등) → 미완료, 다음 평일 재시도
        return False
    if not plan["buys"]:           # 신규 편입할 종목 없음 = 이미 목표 보유 → 이번 주 할 일 없음
        return True
