...
def _rebalance(acct, symbol, target_w, prices, band):
    ...
    logger.info(f"{side} {symbol} qty={quantity} acct={acct} (target={target_w:.3f})")
...
def run() -> None:
    ...
    logger.info(f"started — {TOPIC_SIGNALS} → place_order (모의), roster={roster}, band={band}")
    try:
        while True:
            ...
            try:
                ...
                logger.warning(f"skip bad signal: {e}")
                continue
            ...
            logger.info(f"combined {symbol} target={combined:.4f} (bar={bar_ts}, loads={len(roster)})")
    finally:
        consumer.close()
        close_pool()
