...
class TokenCache:
    ...
    def _save(self) -> None:
        try:
            ...
            logger.error(f"[{self._name}] 토큰 캐시 저장 실패({type(e).__name__}: {e}) — "
                  f"프로세스 간 토큰 재사용 불가, KIS 발급한도 소모 주의")
    ...
    def get(self, force: bool = False) -> str:
        ...
        with self._lock:
            ...
            if force or self._token is None or now >= self._expires_at - self._margin:
                ...
                self._save()
                logger.info(f"[{self._name}] 접근토큰 발급 (만료 {self._expires_at.isoformat()})")
            return self._token
