...
- VM: `coin-trader-vm`, `e2-small`(2vCPU 공유/2GB), `pd-standard` 30GB, Ubuntu 22.04 (구 e2-medium에서 collector 전용화하며 다운사이즈)
...
> ⚠ 이 "공개 HTTPS" 절은 **과거 상시 공개 대시보드 시절의 절차 기록**이다. 현행 운영은 공개 URL 없이
> 매매 VM 온디맨드 대시보드 모드 + SSH 터널이다(§상단 요약). 공개가 다시 필요할 때만 참고.
