...
>   - **`trade-vm-daily`**: 매일 01:00 UTC(KST 10:00) → 코인 `trade_once`(매일) + **KR 주식**(평일마다 시도, KR장중) → poweroff.
>   - **`trade-vm-us-close`**: 평일 15:00 ET(tz=America/New_York, DST 자동 → 19:00 UTC 서머/20:00 UTC 윈터) → **US 주식**(`us_trade_once`, 미국장 막바지=장중 체결) → poweroff. startup 분기: hour∈{19,20,21}=US만, 그 외=코인+KR.
>   - **주 1회 멱등(휴장·실패 재시도)**: 주식은 평일마다 부팅·시도하지만 `weekly_rebalance` 마커가 **주 1회만 실제 매매**를 보장. 휴장일(US=NYSE 게이트, KR=체결기반)·일시적 실패면 그 주 마커를 남기지 않아 **다음 평일 자동 재시도**.
...
### ⏰ 매매 스케줄러 (Cloud Scheduler → instances/start)
두 잡이 같은 매매 VM을 기동한다. startup이 UTC 부팅시각으로 분기(hour∈{19,20,21}=US, 그 외=코인+KR).
```bash
# US 야간 잡 (평일 15:00 ET, DST 자동). 데일리 잡(trade-vm-daily, 0 1 * * * UTC)은 이미 존재.
# 평일마다 발화하지만 주간 마커(weekly_rebalance)가 주 1회만 실제 매매 — 휴장·실패면 다음 평일 재시도.
SA=689150179824-compute@developer.gserviceaccount.com
URI=https://compute.googleapis.com/compute/v1/projects/coin-auto-trader-jvfhgq/zones/us-central1-a/instances/coin-trade-vm/start
gcloud scheduler jobs create http trade-vm-us-close --location=us-central1 \
  --schedule="0 15 * * 1-5" --time-zone="America/New_York" \
  --uri="$URI" --http-method=POST \
  --oauth-service-account-email="$SA"
```
> **기존 잡 갱신(월→평일)**: `gcloud scheduler jobs update http trade-vm-us-close --location=us-central1 --schedule="0 15 * * 1-5" --time-zone="America/New_York"`. KR은 startup 스크립트가 평일(BOOT_DOW 1-5)에 `stock-trade-once`를 돌리고 마커가 주 1회를 보장(스케줄러 변경 불요).
