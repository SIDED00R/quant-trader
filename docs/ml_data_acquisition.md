...
- `index_changes` (date, symbol, index_name, action(add/drop), source) — 편입편출 이벤트. **구현됨**(`batch/rawdata/us_membership.py`·`kr_index_membership.py` 적재, `db/clickhouse_schema.sql`). *(발표일 vs 적용일 분리는 미구현 — 단일 `date`만.)*
