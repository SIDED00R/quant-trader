...
**해소**: 무료 KRX 데이터포털 계정(data.krx.co.kr 회원가입) 자격증명을 `KRX_ID/KRX_PW`(.env)에 설정 → 실조회 검증 완료(투자자별 12분류 수급·외국인 한도소진율·공매도 잔고/거래량·KOSPI200/KOSDAQ150 구성 전부 정상). 수집기 `batch/data/krx.py` 구현(도메인 1·2 적재).
...
| 변수 | 출처 | obtainable | 이력 | 우선 |
|---|---|---|---|---|
| KOSPI200/KOSDAQ150 PIT | pykrx index_portfolio_deposit_file | KRX(로그인) ✅구현(`batch/data/kr_index_membership.py`) | ~2014~ | **high** |
| S&P500 PIT | GitHub fja05680/sp500 (MIT) | 키리스 | 1996~ | **high** |
| NASDAQ100 PIT | 무료 정형 PIT 소스 부재 → 현재구성(Wikipedia 스냅샷) 근사만 | — | — | low(미적재 — `batch/data/us_membership.py` 한계 명시) |
...
| 변수 | 출처 | obtainable | 이력 | 우선 |
|---|---|---|---|---|
| PBR, ROA | KR DART / US EDGAR + 주가 | DART키 ✅(`batch/data/kr_fundamentals.py`) / EDGAR키리스 | KR2015~/US2009~ | **high**(난이도 최저) |
...
- `index_changes` (date, symbol, index_name, action(add/drop), source) — 편입편출 이벤트. **구현됨**(`batch/data/us_membership.py`·`kr_index_membership.py` 적재, `db/clickhouse_schema.sql`). *(발표일 vs 적용일 분리는 미구현 — 단일 `date`만.)*
