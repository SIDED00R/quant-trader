...
- `index_changes` (announce_date, effective_date, symbol, index_name, action(add/drop), source) — 편입편출 이벤트(발표일 vs 적용일 분리). **구현됨**(us_membership.py 적재, clickhouse_schema.sql)
