...
**외부데이터 누적(US lambdarank 5시드6fold) — 펀더멘털·13F가 점진 개선, 매크로는 제외:**
| US config | Rank IC | ICIR | NW_t | 비고 |
|---|---|---|---|---|
| OHLCV 전용 (58) | 3.09% | 0.171 | 1.8 | 베이스 |
| + 펀더멘털 (66) | 3.41% | 0.195 | 2.1 | +0.32%p, 유의성 돌파 |
| + 펀더멘털 + 13F (70) | 3.56% | 0.200 | 2.2 | +0.15%p |
| **+ 펀더멘털 + 13F + 섹터(indmom)** | **3.78%** | — | **2.3** | **+0.22%p, 현 챔피언**(ml_progress §3) |
| + 매크로 | 2.05% | 0.137 | 1.5 | 과적합 → 제외 |

→ **검증된 외부데이터(펀더멘털·13F)는 누적적으로 IC를 올린다**(NW_t 1.8→2.1→2.2). 13F는 기관 보유수·총주식수 QoQ가 기여(중요도 top). **확정 US 챔피언 피처셋 = OHLCV+펀더멘털+13F+섹터(indmom)**(3.78%, ml_progress §3 채택). 매크로(시장수준)만 제외. KR은 외국인수급(KRX 키)으로 동일 기대.
...
- **현 챔피언 = GBDT(OHLCV+펀더멘털+13F+섹터, ~3.78%/NW_t 2.3)**. DL은 GPU 풀스케일에서 재평가(서베이 prior: tabular는 GBDT가 강함 — DL이 이기려면 raw 시퀀스 풀학습 필요). GPU 투자 전 KR 외국인수급(키) 추가가 ROI 더 클 수 있음.
