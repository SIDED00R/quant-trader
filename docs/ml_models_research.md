...
| **+ 펀더멘털 + 13F (70)** | **3.56%** | **0.200** | **2.2** | **+0.15%p, 현 최선** |
| + 매크로 | 2.05% | 0.137 | 1.5 | 과적합 → 제외 |

→ **검증된 외부데이터(펀더멘털·13F)는 누적적으로 IC를 올린다**(NW_t 1.8→2.1→2.2). 13F는 기관 보유수·총주식수 QoQ가 기여(중요도 top). **확정 US 피처셋 = OHLCV+펀더멘털+13F**. 매크로(시장수준)만 제외. KR은 외국인수급(KRX 키)으로 동일 기대.
