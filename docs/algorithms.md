...
## 공통 프레임 (`strategy/plugins/disciplined.py`)
...
- 지표 계산은 순수 함수 `strategy/core/indicators.py`(rsi/bollinger/donchian/Ema)로 분리해 단위 테스트한다.
...
- **구현**(`strategy/plugins/rsi.py`, `indicators.rsi`): period=14, 과매도 ≤30 → BUY, 과매수 ≥70 → SELL. 단순(SMA식) RSI.
...
- **구현**(`strategy/plugins/macd.py`, `indicators.Ema`): EMA는 경로 의존이라 종목별 증분 상태로 유지. MACD선이 시그널선을 **상향 교차 → BUY**, **하향 교차 → SELL**. 수렴 전(26봉 미만) 신호 억제.
...
- **구현**(`strategy/plugins/bollinger.py`, `indicators.bollinger`): window=20, k=2.0. 하단(SMA−2σ) 이하 → BUY, 상단(SMA+2σ) 이상 → SELL.
...
- **구현**(`strategy/plugins/breakout.py`, `indicators.donchian`): lookback=20(현재 봉 제외). 직전 최고가 초과 → BUY, 직전 최저가 미만 → SELL.
...
|---|---|---|---|---|
| RSI | `strategy/plugins/rsi.py` | `RSIStrategy` | `rsi` | 평균회귀 |
| MACD | `strategy/plugins/macd.py` | `MACDStrategy` | `macd` | 추세추종 |
| 볼린저 | `strategy/plugins/bollinger.py` | `BollingerStrategy` | `bollinger` | 평균회귀 |
| 돌파 | `strategy/plugins/breakout.py` | `BreakoutStrategy` | `breakout` | 추세추종 |

레지스트리(`strategy/core/registry.py`)에 등록되어 `python -m batch.backtest.run --strategy <이름>`으로 선택한다.
