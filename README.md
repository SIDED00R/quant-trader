...
| `infra/` | GCP 기동 스크립트(`collector-vm-startup.sh`=틱 수집 상시 · `trade-vm-startup.sh`=온디맨드 자기완결·대시보드 모드) + 배포(`deploy-collector-vm.sh`) + CI/CD·스케줄러 1회 셋업(`setup-cicd.sh`). VM on/off·대시보드는 별도 `gcp-cost-controller` 프로젝트(텔레그램)가 담당 |
