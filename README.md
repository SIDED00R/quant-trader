...
| **전략** | [주식] LightGBM 횡단면 챔피언(KR=OHLCV+DART 펀더 / US=+13F·섹터) 주간 top-N · [KR 일목] 주봉 구름 돌파 페이퍼(ML과 자산곡선 비교) · [코인] 일봉 저회전 추세추종 앙상블(5/40·10/60·20/100), walk-forward·Deflated Sharpe 검증 |
...
| `ichimoku.py` | 일목균형표 순수 지표(일봉→주봉 리샘플·전환/기준/선행스팬·최신 신호) — batch 비의존(app 공용) |
| `toss_daily.py` | 토스 일봉 fetch(REST, 401 자가재발급) — app 이미지 공용(/차트 봇 온디맨드). CH 적재는 `batch.backtest.toss_daily` |
| `candle_chart.py` · `symbol_chart.py` | 캔들+일목 구름 PNG 렌더(pillow, ASCII) / 종목 1개→차트+한글 캡션 조립(KR 주봉+일목·US 일봉) |
| `stock_names.py` | 종목명↔티커 조회(US=SEC company_tickers·KR=KRX best-effort)·질의 해석 — /차트 봇·검색용(정확 티커는 사전 무관 동작) |
...
| `strategy/kr_ichimoku_trade_once.py` | KR 일목 주봉 구름 돌파 **페이퍼 매매**(진입=종가>구름상단 AND 전환>기준 / 청산=전환선 데드크로스 / 캡 30·돌파강도 우선). 실주문 없이 시뮬 장부 기록, ML 전략과 자산곡선 병행 비교. 순수 결정부 `plan_trades`. *(Dockerfile.batch — trade 프로파일)* |
...
| `backtest/upbit_candles.py`·`upbit_daily.py`·`toss_daily.py`·`toss_intraday.py` | Upbit 분봉/일봉·Toss 일봉(CH 적재+`common.toss_daily` fetch re-export)/분봉 수집 |
...
| `backtest/tests/` | 단위테스트(319개, 일목 지표·페이퍼 장부·페이퍼 매매 계획·캔들 렌더·종목명·/차트 봇 포함) |
...
| 폴더 | 역할 |
|------|------|
| `api/` | FastAPI 대시보드/REST — `main.py`(앱), `security.py`/`auth_google.py`(인증), `cache.py`/`warmup.py`(TTL 캐시+기동 예열), `stock_order_executor.py`(수동주문 예약 실행기), `telegram_bot.py`(/차트 봉차트 봇 — 수집 VM 상시, Bot API long-poll), `routes/`(account·orders·market·history·performance·strategy·decisions·equity·autotrade·stocks·stock_orders·rebalance·health·web) |
| `scripts/` | `init_db.py`(스키마 1회 적용)·`reset_account.py`(모의 계정 리셋)·`render_equity_chart.py`(자산 차트 SVG 렌더 — stdlib, assets 브랜치 발행용)·`telegram_login.py`(텔레그램 StringSession 1회 발급) |
...
- `collector` — 상시 틱 수집 VM(Kafka+ClickHouse+Postgres+WS 레코더 + 텔레그램 `/차트` 봇; 대시보드·매매 제외 → e2-small)
- `data` — (구) 데이터 VM 풀 서브셋(수집·저장·대시보드) — 로컬/레거시용
- `trade` — 온디맨드 매매(코인 `trade_once` + KR/US 주식 `stock_trade_once`·`us_trade_once` + KR 일목 페이퍼 `kr-ichimoku-trade-once` + `maintenance_once` + 자산 차트 `equity-chart`); **로컬 DB 자기완결**(터널 없음)
