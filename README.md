...
| `toss_daily.py` | 토스 일봉 fetch(REST, 401 자가재발급) — app 이미지 공용(관심종목 푸시 Toss 폴백). CH 적재는 `batch.backtest.toss_daily` |
| `candle_chart.py` · `symbol_chart.py` | 캔들+일목 구름 PNG 렌더(pillow, ASCII) / 종목 1개→차트+한글 캡션 조립(KR 주봉+일목·US 일봉) |
| `stock_names.py` | 종목명↔티커 조회(US=SEC company_tickers·KR=KRX best-effort)·CH `stock_names` 적재 — 관심종목 검색용 |
...
| `backtest/tests/` | 단위테스트(일목 지표·페이퍼 장부·페이퍼 매매 계획·캔들 렌더·종목명·관심종목 검색/푸시 포함) |
...
| 폴더 | 역할 |
|------|------|
| `api/` | FastAPI 대시보드/REST — `main.py`(앱), `security.py`/`auth_google.py`(인증), `cache.py`/`warmup.py`(TTL 캐시+기동 예열), `stock_order_executor.py`(수동주문 예약 실행기), `routes/`(account·orders·market·history·performance·strategy·decisions·equity·autotrade·stocks·stock_orders·rebalance·watchlist·health·web) |
...
- `collector` — 상시 틱 수집 VM(Kafka+ClickHouse+Postgres+WS 레코더; 대시보드·매매 제외 → e2-small)
