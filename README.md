...
| `log.py` | 로깅 표준 설정(`setup()` — 타임스탬프+레벨+로거명, `LOG_LEVEL` env). 데몬·잡 진입점에서 1회 호출 |
...
| 폴더 | 역할 |
|------|------|
| `tests/` | 단위테스트(소스 트리 미러: `tests/{backtest,candles,trading,common,api,scripts,jobs}/`) + 공용 `helpers.py`(fake_pool). 자금경로(브로커 추격·취소·주문실행·잔고동기화·rate_limit·자동매매 API) 커버 |
