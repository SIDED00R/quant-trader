...
| **배포** | GCP **2-VM** — 상시 데이터 VM + 온디맨드 매매 VM(스케줄러 6잡: 코인 매일 10:00 KST / KR 평일 15:00 KST / US 평일 15:30 ET + 각 스위퍼 재시도) · main 머지 시 CI/CD 자동배포 |
...
GCP **2-VM** — 상시 데이터 VM(수집·저장·대시보드) + 온디맨드 매매 VM. Cloud Scheduler **6잡**(메인 3+기동실패 재시도 스위퍼 3)이 같은 VM을 기동(부팅 시각으로 분기): 코인 **매일 10:00 KST**, KR 주식 **평일 15:00 KST**, US 주식 **평일 15:30 ET**(마감 30분 전, DST 자동). 각 매매 후 자가 종료(~$25/월). **main 머지 = 자동 배포**(GitHub Actions → Artifact Registry → 양 VM). 절차는 [DEPLOY.md](DEPLOY.md).
