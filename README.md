...
| `data/fundamentals.py`·`fred.py`·`us_membership.py`·`sec_13f.py`·`sec_sector.py`·`krx.py`·`krx_bulk.py`·`kr_index_membership.py`·`kr_fundamentals.py` | **외부데이터 영구수집**(멱등·일별증분): SEC EDGAR 펀더멘털·FRED 매크로·US 지수 PIT멤버십·13F 기관보유·SEC SIC 섹터·KR 수급/공매도/외국인보유(KRX, `krx_bulk`=고속 by-date)·KR 지수 PIT멤버십·KR 펀더멘털(DART) |
