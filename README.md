...
| `strategy/kr_ichimoku_trade_once.py` | KR 일목 주봉 구름 돌파 **페이퍼 매매**(진입=종가>구름상단 AND 전환>기준 / 청산=전환선 데드크로스 / 캡 30·돌파강도 우선). 실주문 없이 시뮬 장부 기록, ML 전략과 자산곡선 병행 비교. 순수 결정부 `plan_trades`. 매매 후 **신규 매수 종목의 주봉+일목 차트를 텔레그램 사진으로 발송**(`send_entry_charts`, /차트 봇 렌더러 재사용). *(Dockerfile.batch — trade 프로파일)* |
...
| `backtest/upbit_candles.py`·`upbit_daily.py`·`toss_daily.py`·`toss_intraday.py` | Upbit 분봉/일봉·Toss 일봉(CH 적재+`common.marketdata.toss_daily` fetch re-export)/분봉 수집 |
...
| (최상위) `tests/` | 단위테스트(소스 트리 미러: `tests/{backtest,candles,trading,common,api,scripts}/`) |
