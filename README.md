...
| **배포** | GCP **2-VM** — 상시 데이터 VM + 온디맨드 매매 VM(스케줄러 8잡: 코인 매일 10:00 KST / KR 평일 15:00 KST / US 평일 15:30 ET / 데이터 유지보수 매월 첫 토 04:00 UTC + 각 스위퍼 재시도) · main 머지 시 CI/CD 자동배포 |
...
| `infra/` | GCP 기동 스크립트(`gce-startup.sh`·`trade-vm-startup.sh`) + 배포(`deploy-data-vm.sh`) + CI/CD·스케줄러 1회 셋업(`setup-cicd.sh`) |
