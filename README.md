...
| **배포** | GCP **2-VM** — 상시 **틱 수집 VM(collector, e2-small)** + 온디맨드 **매매 VM(자기완결·로컬 DB)**(스케줄러 8잡: 코인 매일 10:00 KST / KR 평일 15:00 KST / US 평일 15:30 ET / 데이터 유지보수 매월 첫 토 04:00 UTC + 각 스위퍼 재시도) · 매매 VM은 텔레그램 봇으로 수동 on/off · main 머지 시 CI/CD 자동배포 |
...
| `backtest/tests/` | 단위테스트(224개) |
| `data/fundamentals.py`·`fred.py`·`us_membership.py`·`sec_13f.py`·`sec_sector.py`·`krx.py`·`krx_bulk.py`·`kr_index_membership.py`·`kr_fundamentals.py`·`kr_delisted.py`·`_krx_session.py` | **외부데이터 영구수집**(멱등·일별증분): SEC EDGAR 펀더멘털·FRED 매크로·US 지수 PIT멤버십·13F 기관보유·SEC SIC 섹터·KR 수급/공매도/외국인보유(KRX, `krx_bulk`=고속 by-date)·KR 지수 PIT멤버십·KR 펀더멘털(DART)·KR 상장폐지+상폐 OHLCV(FDR, 생존편향 보정)·공용 KRX 로그인 세션(`_krx_session`) |
| `data/maintenance_once.py` | 정기 데이터 유지보수 1회 실행 — 활성 유니버스 일봉 풀 재백필 + 위 분기 수집기(EDGAR·13F·SIC·DART) 재실행, 매월 첫 토요일 04:00 UTC |
...
| `ml/dataset.py`·`cv.py`·`evaluate.py`·`baseline_lgbm.py`·`tune_gbdt.py`·`stock_score.py` | **ML 모델링**(주식 횡단면 수익예측): 피처+라벨 조립·purged/embargo CV·Rank IC/ICIR/NW-t 평가·LightGBM(lambdarank, 챔피언)·GBDT 누설없는 튜닝·라이브 트레이더용 최신 거래일 챔피언 스코어러(`stock_score`). 시장별(US/KR) 분리. (DL 비교실험 GRU/MLP은 GBDT에 패배해 코드 삭제 — 기록은 `docs/ml_models_research.md`·`ml_progress.md`) |
...
| `infra/` | GCP 기동 스크립트(`collector-vm-startup.sh`=틱 수집 상시 · `trade-vm-startup.sh`=온디맨드 자기완결 · 구 `gce-startup.sh`) + `telegram-vm-bot/`(매매 VM on/off 봇) + 배포(`deploy-data-vm.sh`) + CI/CD·스케줄러 1회 셋업(`setup-cicd.sh`) |
| `.github/workflows/` | `deploy.yml` — main 머지 시 이미지 빌드→Artifact Registry→VM 자동배포+헬스체크 |
...
8. `stock_trade_once`·`us_trade_once`: 가드(주간마커·휴장) → `refresh_stock_daily`(일봉 증분 갱신) → 스코어링(신선도·커버리지 게이트) → 주문
...
GCP **2-VM** — 상시 데이터 VM(수집·저장·대시보드) + 온디맨드 매매 VM. Cloud Scheduler **8잡**(메인 4+기동실패 재시도 스위퍼 4)이 같은 VM을 기동(부팅 시각으로 분기): 코인 **매일 10:00 KST**, KR 주식 **평일 15:00 KST**, US 주식 **평일 15:30 ET**(마감 30분 전, DST 자동), 데이터 유지보수 **매월 첫 토요일 04:00 UTC**. 각 잡 후 자가 종료(~$25/월). **main 머지 = 자동 배포**(GitHub Actions → Artifact Registry → 양 VM). 절차는 [DEPLOY.md](DEPLOY.md).

**매 실행 텔레그램 통보**: 각 매매 잡이 결과(매수 내역 전부 또는 '매매하지 않음'+사유)와 오류를 텔레그램(MTProto, `common/notify_telegram`)으로 발송한다 — 스킵 실행도 발송하므로 **알림 부재 = 장애 신호**. 잡이 뜨기도 전에 죽는 실패(이미지 빌드 등)는 startup의 `notify_fail` 폴백이 로그 꼬리와 함께 발송(exit 70='파이썬이 이미 통보' 센티널). 자격증명은 Secret Manager `telegram-env`(발급: `scripts/telegram_login.py`).
