...
| `data/maintenance_once.py`·`verify_freshness.py` | 정기 데이터 유지보수 1회 실행 — 활성 유니버스 일봉 풀 재백필 + 위 분기/월간 수집기(EDGAR·13F·SIC·DART·내부자·팩터·US공매도·실적) 재실행 + 마지막에 **데이터 신선도 점검**(`verify_freshness`, 읽기전용: 테이블별 행수·최신일자·지연일; 임계 테이블 낡음/빔 → 🔴), 매월 첫 토요일 04:00 UTC |
