...
| `strategy/stock_trade_once.py` · `us_trade_once.py` | 주식 ML 챔피언 top-N 주간 모의 리밸런싱(KR=KIS 국내 시장가 / US=해외 지정가·거래소 라우팅) *(Dockerfile.batch — trade 프로파일, batch.ml 의존)* |
| `strategy/stock_trade_common.py` | 위 KR/US trade-once 공통부(최신 종가 조회·매매계획·잔고 폴링 체결확인) |
