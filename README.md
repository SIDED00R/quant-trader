...
| **FRED** | 매크로(금리·VIX·환율·유가) | 수동 실행(스케줄 미배선 — 적재분 보존, API 키) |
| **GitHub(fja05680/sp500)** | US 지수 PIT 멤버십·편입편출 | 수동 실행(스케줄 미배선 — 적재분 보존, 생존편향 부분해결) |
| **KRX(pykrx)** | KR 외국인/기관 수급·공매도·외국인보유(연구 데이터) | 수동 실행(스케줄 미배선 — 적재분 보존, 로그인·`batch/data/krx.py`) |
| **DART** | KR 펀더멘털(ML 피처) | 운영(`batch/data/kr_fundamentals.py`) |
| **FDR(FinanceDataReader)** | KR **상장폐지** 종목·상폐 OHLCV(ML 생존편향 보정) | 수동 실행(스케줄 미배선 — 적재분 보존, `batch/data/kr_delisted.py`) |
